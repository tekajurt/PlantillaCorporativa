...
- Accesibilidad:
  ...
  - Nota: el código fuente está en `code/`.
  ...
  ***
  ...
  Si quieres, empiezo por (marca uno):

  - A) Configurar CI + ESLint + Prettier y tests básicos.
  - B) Implementar `/api/contact` y validar flujo del formulario.
  - C) Implementar filtros y modal en la galería.

  Indica la opción y procedo con el siguiente paso del plan (actualizaré la lista TODO y avanzaré en el repo).
